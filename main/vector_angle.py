...
# get atoms from models for each selection
  ...
  print " ", name_start1,s1.atom[0].chain, s1.atom[0].resn,s1.atom[0].resi,s1.atom[0].name, "to ",
  print name_end1,e1.atom[0].chain, e1.atom[0].resn,e1.atom[0].resi,e1.atom[0].name
  print "and"
  print " ", name_start2,s2.atom[0].chain, s2.atom[0].resn,s2.atom[0].resi,s2.atom[0].name, "to ",
  print name_end2,e2.atom[0].chain, e2.atom[0].resn,e2.atom[0].resi,e2.atom[0].name
  print ""
  print "Angle between    [%.3f %.3f %.3f]" % (vector1[0],vector1[1],vector1[2]),
  print "and [%.3f %.3f %.3f] " % (vector2[0],vector2[1],vector2[2]), " is %.2f degrees" % angle(vector1,vector2)
  print "Distance between [%.3f %.3f %.3f]" %  (vector1[0],vector1[1],vector1[2]),
...
if __name__ == "__main__":
  ...
  for input in sys.stdin.readlines():
    if len(input.split()) == 6:
      points = map(float,input.replace(',',' ').split())
      ...
      vector.append(map(float,input.split))
    else:
